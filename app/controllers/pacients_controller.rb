class PacientsController < ApplicationController
  ...
  def index
    if params[:query].present?
      @pacients = Pacient.where("nome like ?","%#{params[:query]}%").order(:nome).page(params[:page]).per(8)
    else
      @pacients = Pacient.order(:nome).page(params[:page]).per(8)
    end
  end
  ...
  private
    ...
    def pacient_params
      params.require(:pacient).permit(:nome, :cpf, :sexo, :email, :telefone, :data_de_nascimento)
    end
end
